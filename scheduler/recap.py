...
def _fetch_leads_by_niche(supa: SupabaseService) -> tuple[dict, date]:
...
def _fetch_scrape_progress(supa: SupabaseService) -> dict:
...
def _build_message(stats: dict, scrape: dict, yesterday: date) -> str:
    lines = []
    ...
    detail = "\n".join(lines) if lines else "Aucun lead extrait hier."
...
def run_daily_recap():
    ...
    stats, yesterday = _fetch_leads_by_niche(supa)
    scrape = _fetch_scrape_progress(supa)

    msg = _build_message(stats, scrape, yesterday)
    try:
